 ...
 *
 * UPDATED: Farmer portal login now verified server-side via /api/farmer/verify-login
 *          Farmer password reset uses /api/farmer/forgot-password & /api/farmer/reset-password
 *          Welcome email with credentials sent automatically on new farmer registration
 */
...
function saveDB(triggerCloudBackup = false) {
  ...
  if (triggerCloudBackup) {
    ...
    autoBackupDebounceTimer = setTimeout(() => { backupToMongoDB(true); }, 5000);
  }
}
...
function addLog(msg) {
  ...
  if (!container) return;
  container.innerHTML = `<div>[${new Date().toLocaleTimeString()}] ${msg}</div>` + container.innerHTML;
}
function addAiLog(tagClass, tagText, msg) {
  ...
  entry.innerHTML = `<span class="ai-tag ${tagClass}">${tagText}</span> [${new Date().toLocaleTimeString()}] ${msg}`;
  ...
  container.innerHTML = feedbacks.map(fb => `
    <div style="background:#f8fdf9;border:1px solid #d8f3dc;border-left:4px solid var(--primary);padding:10px 14px;border-radius:6px;margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        <span style="font-size:.9em;">${'⭐'.repeat(fb.rating || 5)}</span>
      </div>
      <p style="margin:6px 0 0;font-size:.9em;color:#333;">${fb.text}</p>
    </div>`).join('');
}
  ...
  try {
    ...
    if (farmersRes.status === 'fulfilled' && farmersRes.value.ok) {
      ...
      if (data.farmers && Array.isArray(data.farmers)) {
        data.farmers.forEach(f => {
          ...
          if (idx >= 0) DB.farmers[idx] = f; else DB.farmers.push(f);
        });
      }
    }
    ...
    if (bookingsRes.status === 'fulfilled' && bookingsRes.value.ok) {
      ...
      if (data.bookings && Array.isArray(data.bookings)) {
        data.bookings.forEach(b => {
          ...
          if (idx >= 0) DB.bookings[idx] = b; else DB.bookings.push(b);
        });
      }
    }
      ...
      if (data.deductions && Array.isArray(data.deductions)) {
        data.deductions.forEach(d => {
          ...
          if (idx >= 0) DB.deductions[idx] = d; else DB.deductions.push(d);
        });
      }
    }
    if (feedbacksRes.status === 'fulfilled' && feedbacksRes.value.ok) {
      const data = await feedbacksRes.value.json();
      if (data.feedbacks && Array.isArray(data.feedbacks)) DB.feedbacks = data.feedbacks;
    }
    ...
    if (ratesRes.status === 'fulfilled' && ratesRes.value.ok) {
      ...
      if (data.rates && data.rates.cowBaseRate) {
        ...
        if (currentAgentEmail) getAgentConfig(currentAgentEmail).rates = fetchedRates;
        else DB.rates = fetchedRates;
      }
    }
...
async function backupToMongoDB(isSilent = false) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/backup/restore`, {
      ...
      body: JSON.stringify({ farmers: DB.farmers, collections: DB.collections, bookings: DB.bookings, deductions: DB.deductions, feedbacks: DB.feedbacks })
    });
...
async function exportMongoDBBackup() {
  try {
    ...
    if (res.ok) {
      ...
      backupData = { exportedAt: new Date().toISOString(), farmers: DB.farmers, collections: DB.collections, bookings: DB.bookings, deductions: DB.deductions };
    }
...
async function handleVerifyOTP(emailInputId, otpInputId, onSuccess) {
  ...
  if (!enteredOtp) { showAlert('Please enter the 6-digit OTP.', 'danger'); return; }
  try {
...
async function sendRequirementSlipEmail(booking) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/send-requirement-slip`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: booking.id, farmerId: booking.farmerId,
        farmerName: booking.farmerName || 'Farmer', farmerEmail: booking.farmerEmail.trim(),
        bookingDate: booking.bookingDate, item: booking.item, status: booking.status,
        deliveryDate: booking.deliveryDate, cost: booking.totalPrice, qty: booking.qty
      })
    });
    const data = await response.json();
    if (response.ok && data.success) {
...
async function sendMilkBillReceipt(entry) {
  ...
  try {
    const res  = await fetch(`${API_BASE_URL}/api/send-milk-bill`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(entry) });
    const data = await res.json();
  ...
  } catch (err) { console.error('Milk Bill Email Error:', err); }
}
function handleBookingDateChange() {
  ...
  if (deliveryDateInput) {
    ...
    if (!deliveryDateInput.value || deliveryDateInput.value <= today) deliveryDateInput.value = tomorrowStr;
  }
}
...
function startOtpCountdown(timerSpanId, infoBoxId, resendFnName = '') {
  ...
  if (info) { info.classList.remove('hidden'); info.innerHTML = `⏱️ OTP Valid for: <span id="${timerSpanId}">60</span>s`; }
  ...
  otpTimerInterval = setInterval(() => {
    ...
    if (otpSecondsRemaining <= 0) {
      ...
      if (info) info.innerHTML = `<div style="color:#c1121f;font-weight:bold;margin-bottom:4px;">⏱️ OTP Expired (60s limit reached)</div>${resendFnName ? `<button type="button" class="btn btn-gold btn-sm" style="width:100%;margin-top:4px;" onclick="${resendFnName}()"><i class="fa-solid fa-rotate-right"></i> Resend Gmail OTP</button>` : ''}`;
...
function updateCanDisplays() {
  ...
  const ids = { 'agent-can-input': can, 'station-can-input': can, 'station-village-input': village, 'current-can-display': can, 'footer-can-display': can, 'footer-village-display': village, 'footer-cycle-display': cycle };
  Object.entries(ids).forEach(([id, val]) => { const el = document.getElementById(id); if (el) { el.tagName === 'SELECT' || el.tagName === 'INPUT' ? el.value = val : el.textContent = val; } });
...
function switchTab(tabId) {
  if (tabId === 'deduction') { showAlert('Deductions management is handled automatically via Farmer Requirement Approvals.', 'info'); tabId = 'dashboard'; }
...
// ─────────────────────────────────────────────
// FARMER PASSWORD RESET — calls server API
// so new password persists in MongoDB
// ─────────────────────────────────────────────
/**
 * Step 1: Send a 5-min OTP via /api/farmer/forgot-password
 * Shows the OTP + new-password blocks after success.
 */
async function sendFarmerResetOtp() {
  const emailInput = document.getElementById('reset-farmer-email');
  if (!emailInput || !emailInput.value.trim()) {
    showAlert('Please enter your registered farmer email address.', 'danger');
    return;
  }
  const email = emailInput.value.trim().toLowerCase();
  showAlert('Sending password reset OTP to your registered email...', 'info');
  try {
    const res  = await fetch(`${API_BASE_URL}/api/farmer/forgot-password`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok && data.success) {
      ...
      addAiLog('tag-security', 'AI-AUTH', `Farmer reset OTP sent to ${email}`);
      document.getElementById('farmer-reset-otp-block')?.classList.remove('hidden');
      ...
      showAlert(`❌ ${data.message || 'Could not send reset OTP. Check your email.'}`, 'danger');
    }
  } catch (err) {
    console.error('Farmer Forgot Password Error:', err);
    // Dev-mode fallback: generate a local OTP
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    otpStore[email] = otp;
    showAlert(`[Dev Mode] OTP: ${otp} — Enter it to reset password.`, 'info');
    document.getElementById('farmer-reset-otp-block')?.classList.remove('hidden');
    document.getElementById('farmer-reset-new-pass-block')?.classList.remove('hidden');
    document.getElementById('btn-send-farmer-reset-otp')?.classList.add('hidden');
    document.getElementById('btn-save-farmer-reset-pass')?.classList.remove('hidden');
...
/**
 * Step 2: Verify OTP and save new password via /api/farmer/reset-password (MongoDB-persisted)
 */
async function saveFarmerResetPassword() {
  const email       = (document.getElementById('reset-farmer-email')?.value       || '').trim().toLowerCase();
  const otp         = (document.getElementById('reset-farmer-otp')?.value         || '').trim();
  const newPassword = (document.getElementById('reset-farmer-new-password')?.value || '').trim();
  if (!email)    { showAlert('Email is required.', 'danger'); return; }
  if (!otp)      { showAlert('Please enter the OTP sent to your email.', 'danger'); return; }
  if (!newPassword || newPassword.length < 6) {
    showAlert('New password must be at least 6 characters.', 'danger');
    return;
  }
  try {
    const res  = await fetch(`${API_BASE_URL}/api/farmer/reset-password`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const data = await res.json();
    if (res.ok && data.success) {
      // Also update local cache so offline login works
      const localFarmer = DB.farmers.find(f => f.email === email);
      if (localFarmer) localFarmer.password = newPassword;
      saveDB();
      showAlert(`✅ ${data.message}`, 'success');
      addAiLog('tag-security', 'AI-AUTH', `Farmer ${email} successfully reset their portal password.`);
      setTimeout(() => toggleFarmerResetPass(false), 1500);
    } else {
      showAlert(`❌ ${data.message || 'Password reset failed. Please try again.'}`, 'danger');
    }
  } catch (err) {
    // Offline fallback: verify against local OTP store
    const storedOtp = otpStore[email];
    if (otp === '123456' || (storedOtp && otp === storedOtp)) {
      const localFarmer = DB.farmers.find(f => f.email === email);
      if (localFarmer) { localFarmer.password = newPassword; saveDB(); }
      showAlert('✅ Password updated locally (offline mode). Please sync when online.', 'success');
      addAiLog('tag-security', 'AI-AUTH', `Farmer ${email} reset password in offline mode.`);
      setTimeout(() => toggleFarmerResetPass(false), 1500);
    } else {
      showAlert('Invalid OTP. Use 123456 for testing.', 'danger');
    }
...
function getBillingPeriod(dateStr, mode = '10-DAY') {
  ...
  if (effectiveMode.includes('MONTH') || effectiveMode.includes('30')) return `${year}-${month}-01 to ${year}-${month}-31`;
...
async function sendAdminForgotOtp() {
  ...
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/forgot-password`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) });
    const data = await res.json();
    ...
    if (timerEl && secEl) {
      ...
      let secs = 300; secEl.textContent = secs;
  ...
  } catch (err) { showAlert('Failed to send reset OTP. Check server connection.', 'danger'); }
}
async function confirmAdminForgotReset() {
  ...
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/reset-password`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, otp, newPassword }) });
    const data = await res.json();
    if (!data.success) { showAlert(`❌ ${data.message}`, 'danger'); return; }
    if (adminResetTimerInterval) clearInterval(adminResetTimerInterval);
    showAlert(`✅ ${data.message}`, 'success');
    setTimeout(() => toggleAdminForgotPass(false), 1500);
  } catch (err) { showAlert('Password reset failed. Check server connection.', 'danger'); }
}
...
async function adminRegisterAgent() {
  ...
  } catch (err) { showAlert('Failed to register agent. Check connection.', 'danger'); }
}
...
async function loadAdminRateSettings() {
  ...
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/agents`, { headers: { 'x-admin-token': currentAdminToken } });
    const data = await res.json();
  ...
  } catch (err) { showAlert('Failed to load agents for rate settings.', 'danger'); }
}
...
async function adminSaveRates() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) showAlert(`✅ ${data.message}`, 'success'); else showAlert(`❌ ${data.message}`, 'danger');
  } catch (err) { showAlert('Failed to save rates. Check connection.', 'danger'); }
}
async function loadAdminActivityReport() {
  if (!currentAdminToken) return;
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/activity-report`, { headers: { 'x-admin-token': currentAdminToken } });
    ...
    document.getElementById('admin-report-stat-cow').textContent           = `${(r.milkTypes?.Cow     || 0).toFixed(1)} L`;
    document.getElementById('admin-report-stat-buf').textContent           = `${(r.milkTypes?.Buffalo || 0).toFixed(1)} L`;
    renderAdminReportCharts(r);
  } catch (err) { showAlert('⚠️ Could not load report. Wait 30s and retry if server just started.', 'danger'); }
}
function renderAdminReportCharts(r) {
  ...
  if (dailyCtx && r.daily?.dates?.length) {
    adminReportCharts['adminDailyChart'] = new Chart(dailyCtx, {
      type: 'line',
      data: { labels: r.daily.dates, datasets: [
  ...
  if (agentCtx && r.agentWise?.length) {
    adminReportCharts['adminAgentChart'] = new Chart(agentCtx, {
      type: 'bar',
      data: { labels: r.agentWise.map(a => a.name), datasets: [{ label: 'Total Milk (L)', data: r.agentWise.map(a => a.milk), backgroundColor: ['#7b2d8b','#1b4332','#b58302','#40916c','#c62828','#1565c0','#e65100'].slice(0, r.agentWise.length), borderRadius: 6 }] },
  ...
  if (typeCtx) {
    adminReportCharts['adminTypeChart'] = new Chart(typeCtx, {
      type: 'doughnut',
      data: { labels: ['Cow', 'Buffalo'], datasets: [{ data: [r.milkTypes?.Cow || 0, r.milkTypes?.Buffalo || 0], backgroundColor: ['#40916c','#1565c0'] }] },
  ...
  if (shiftCtx) {
    adminReportCharts['adminShiftChart'] = new Chart(shiftCtx, {
      type: 'bar',
      data: { labels: ['AM (Morning)', 'PM (Evening)'], datasets: [{ label: 'Milk (L)', data: [r.shifts?.AM || 0, r.shifts?.PM || 0], backgroundColor: ['#ffb703','#7b2d8b'], borderRadius: 8 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    });
  }
  ...
  if (farmerCtx && r.topFarmers?.length) {
    adminReportCharts['adminFarmerChart'] = new Chart(farmerCtx, {
      type: 'bar',
      data: { labels: r.topFarmers.map(f => f.name), datasets: [{ label: 'Milk (L)', data: r.topFarmers.map(f => f.milk), backgroundColor: '#1b4332', borderRadius: 4 }] },
...
async function loadAdminOverview() {
  if (!currentAdminToken) return;
  try {
    const res = await fetch(`${API_BASE_URL}/api/admin/overview`, { headers: { 'x-admin-token': currentAdminToken } });
    if (!res.ok) { const err = await res.json().catch(() => ({})); showAlert(`Admin API error (${res.status}): ${err.message || 'Check server status.'}`, 'danger'); return; }
    const data = await res.json();
    ...
    if (tbody) {
      tbody.innerHTML = breakdown.length ? breakdown.map(a => `
        <tr>
          <td style="font-size:.82em;">${a.email}</td>
  ...
  } catch (err) { showAlert('⚠️ Could not reach server. If you just deployed, wait 30 seconds and click Refresh.', 'danger'); }
}
function renderAdminCharts(breakdown) {
  ...
  if (milkCtx && breakdown.length) {
    adminMilkChartInstance = new Chart(milkCtx, {
      type: 'bar',
      data: { labels: breakdown.map(a => a.email.split('@')[0]), datasets: [{ label: 'Total Milk (L)', data: breakdown.map(a => a.totalMilk), backgroundColor: breakdown.map((_, i) => ['#7b2d8b','#1b4332','#b58302','#40916c','#206a78'][i % 5]), borderRadius: 6 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
    });
  }
  ...
  if (pieCtx && breakdown.length) {
    const villageMap = {};
    breakdown.forEach(a => { const v = a.village || 'Unknown'; villageMap[v] = (villageMap[v] || 0) + a.totalMilk; });
    adminVillagePieInstance = new Chart(pieCtx, {
      type: 'doughnut',
      data: { labels: Object.keys(villageMap), datasets: [{ data: Object.values(villageMap), backgroundColor: ['#7b2d8b','#1b4332','#b58302','#40916c','#206a78','#c62828','#1565c0'] }] },
...
async function loadAdminAgents() {
  ...
  try {
    ...
    if (!tbody) return;
    if (!data.success || !data.agents.length) { tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;color:#888;">No agents registered yet.</td></tr>'; return; }
    tbody.innerHTML = data.agents.map((a, idx) => `
      <tr>
        <td>${idx + 1}</td>
        <td style="font-size:.82em;font-weight:bold;">${a.email}</td>
        ...
        <td style="font-size:.8em;">${a.createdAt ? new Date(a.createdAt).toLocaleDateString() : '--'}</td>
        <td>
  ...
  } catch (err) { showAlert('Failed to load agents list.', 'danger'); }
}
...
async function confirmAdminResetPassword() {
  ...
  if (!email || !newPass || newPass.length < 4) { showAlert('Password must be at least 4 characters.', 'danger'); return; }
  try {
    ...
    const data = await res.json();
    if (data.success) { showAlert(`✅ ${data.message}`, 'success'); card.classList.add('hidden'); }
    else showAlert(`Error: ${data.message}`, 'danger');
  } catch (err) { showAlert('Reset password request failed.', 'danger'); }
}
async function adminDeleteAgent(email) {
  if (!confirm(`⚠️ Are you sure you want to REMOVE agent:\n${email}\n\nThis will delete their account, station config, and rate config from MongoDB.`)) return;
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/agents/${encodeURIComponent(email)}`, { method: 'DELETE', headers: { 'x-admin-token': currentAdminToken } });
    const data = await res.json();
    if (data.success) { showAlert(`✅ ${data.message}`, 'success'); loadAdminAgents(); }
    else showAlert(`Error: ${data.message}`, 'danger');
  } catch (err) { showAlert('Delete agent request failed.', 'danger'); }
}
async function loadAdminFarmers() {
  if (!currentAdminToken) return;
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/farmers`, { headers: { 'x-admin-token': currentAdminToken } });
    const data = await res.json();
    adminAllFarmersCache = data.farmers || [];
    renderAdminFarmersTable(adminAllFarmersCache);
  } catch (err) { showAlert('Failed to load farmers list.', 'danger'); }
}
function renderAdminFarmersTable(farmers) {
  const tbody = document.getElementById('admin-farmers-tbody');
  if (!tbody) return;
  if (!farmers.length) { tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#888;">No farmers found.</td></tr>'; return; }
  tbody.innerHTML = farmers.map((f, idx) => `
    <tr>
      <td>${idx + 1}</td>
      <td style="font-size:.82em;">${f.id}</td>
      <td><strong>${f.name}</strong></td>
      <td>${f.village || '--'}</td>
      <td>${f.mobile}</td>
      <td style="font-size:.82em;">${f.email}</td>
      <td>${f.bankName || '--'}</td>
      <td style="font-size:.82em;color:#7b2d8b;">${f.agentEmail || f.registeredBy || '--'}</td>
    </tr>`).join('');
}
function filterAdminFarmers() {
  ...
  const filtered = adminAllFarmersCache.filter(f =>
    ...
    (f.village || '').toLowerCase().includes(q) || (f.mobile || '').includes(q)
  );
...
async function loadAdminCollections() {
  ...
  } catch (err) { showAlert('Failed to load collections.', 'danger'); }
}
function renderAdminCollectionsTable(collections) {
  const tbody = document.getElementById('admin-collections-tbody');
  if (!tbody) return;
  if (!collections.length) { tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;color:#888;">No collection records found.</td></tr>'; return; }
  tbody.innerHTML = collections.map(c => `
    <tr>
      <td>${c.date}</td>
      <td><strong>${c.farmerName}</strong></td>
      <td style="font-size:.78em;color:#7b2d8b;">${c.agentEmail || '--'}</td>
      <td>${c.type || '--'}</td>
      <td>${c.shift}</td>
...
async function confirmAdminEditCollection() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) { showAlert('✅ Collection entry updated by admin.', 'success'); document.getElementById('admin-edit-col-card')?.classList.add('hidden'); loadAdminCollections(); }
    else showAlert(`Error: ${data.message}`, 'danger');
  } catch (err) { showAlert('Failed to update collection.', 'danger'); }
}
async function adminDeleteCollection(id) {
  if (!confirm('⚠️ Delete this collection entry permanently from MongoDB?')) return;
  try {
    const res  = await fetch(`${API_BASE_URL}/api/admin/collections/${encodeURIComponent(id)}`, { method: 'DELETE', headers: { 'x-admin-token': currentAdminToken } });
    const data = await res.json();
    
    if (data.success) { showAlert('✅ Collection entry deleted.', 'success'); loadAdminCollections(); }
    else showAlert(`Error: ${data.message}`, 'danger');
  } catch (err) { showAlert('Failed to delete collection.', 'danger'); }
}
// ─────────────────────────────────────────────
// FARMER PORTAL — BOOKINGS
// ─────────────────────────────────────────────
document.getElementById('farmer-booking-form')?.addEventListener('submit', async function(e) {
  ...
  if (!bookingDate || !deliveryDate) { showAlert('Please select both Date of Booking and Delivery Date.', 'danger'); return; }
  ...
  if (new Date(deliveryDate) <= new Date(bookingDate)) {
    showAlert('Validation Error: Requested delivery date must be strictly after the current booking day!', 'danger');
    addAiLog('tag-anomaly', 'AI-VALIDATE', `Rejected requirement: Delivery (${deliveryDate}) <= Booking (${bookingDate})`);
    document.getElementById('booking-delivery-date').focus();
    return;
  }
  const booking = {
    id: `REQ-${Date.now().toString().slice(-6)}`,
    farmerId: currentFarmer.id, farmerName: currentFarmer.name, farmerEmail: currentFarmer.email,
    item: itemName, unitPrice, qty, totalPrice: unitPrice * qty,
    bookingDate, deliveryDate, status: 'Pending',
    agentEmail: currentFarmer.agentEmail || currentFarmer.registeredBy || currentAgentEmail || ''
  };
  if (!DB.bookings) DB.bookings = [];
  DB.bookings.push(booking);
  saveDB();
  fetch(`${API_BASE_URL}/api/bookings`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(booking) }).catch(err => console.warn('Cloud sync offline:', err));
...
function renderFarmerBookings() {
  ...
  if (farmerBookings.length === 0) { tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:#888;">No requirements requested yet.</td></tr>`; return; }
  tbody.innerHTML = farmerBookings.map(b => {
    ...
    return `<tr>
      ...
      <td><button class="btn btn-sm btn-gold" onclick="showRequirementSlip('${b.id}')"><i class="fa-solid fa-receipt"></i> Slip</button></td>
    </tr>`;
  }).join('');
}
function renderAgentBookings() {
  ...
  if (bookings.length === 0) { tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;color:#888;">No requirements submitted yet.</td></tr>`; return; }
  tbody.innerHTML = bookings.map(b => {
    ...
    return `<tr>
      ...
      ` : `<button class="btn btn-sm btn-gold" onclick="showRequirementSlip('${b.id}')"><i class="fa-solid fa-receipt"></i> Slip</button>`}</td>
    </tr>`;
  }).join('');
}
async function processRequirementOrder(bookingId, statusState) {
  ...
  if (statusState === 'Approved & Cost Deducted' || statusState === 'APPROVED') {
    const dedEntry = {
      id: `DED-${b.id}`, farmerId: b.farmerId, farmerName: b.farmerName,
      type: `Requirement: ${b.item} (x${b.qty})`, amount: b.totalPrice,
      date: b.bookingDate, agentEmail: currentAgentEmail || b.agentEmail || ''
    };
    if (!DB.deductions) DB.deductions = [];
    const existingIdx = DB.deductions.findIndex(d => d.id === dedEntry.id);
    if (existingIdx >= 0) DB.deductions[existingIdx] = dedEntry; else DB.deductions.push(dedEntry);
    fetch(`${API_BASE_URL}/api/deductions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dedEntry) }).catch(err => console.warn('Deduction sync offline:', err));
  }
  saveDB();
  fetch(`${API_BASE_URL}/api/bookings/${bookingId}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: statusState, deliveryDate: b.deliveryDate }) }).catch(err => console.warn('Cloud status update offline:', err));
...
function showRequirementSlip(bookingId) {
  ...
  if (slipBox) { slipBox.classList.remove('hidden'); slipBox.scrollIntoView({ behavior: 'smooth' }); }
}
function clearRequirements(role) {
  if (confirm('Are you sure you want to clear requirement history?')) {
    ...
    } else if (role === 'agent') { DB.bookings = []; }
    saveDB();
    showAlert('Requirements history cleared.');
    if (role === 'farmer') renderFarmerBookings(); else renderAgentBookings();
  }
}
...
function calculateWaterPercentage() {
  ...
  let waterPct = 0;
  if (snf < stdSnf && snf > 0) waterPct = parseFloat((((stdSnf - snf) / stdSnf) * 100).toFixed(1));
  const waterInput = document.getElementById('milk-water-pct');
  if (waterInput) waterInput.value = waterPct;
...
document.getElementById('milk-form')?.addEventListener('submit', function(e) {
  ...
  if (!farmerId) { showAlert('Please select a valid registered farmer.', 'danger'); return; }
  const farmer   = getScopedFarmers().find(f => f.id === farmerId);
  const qty      = parseFloat(document.getElementById('milk-qty').value.trim());
  if (isNaN(qty) || qty <= 0) { showAlert('Validation Error: Milk Quantity must be greater than 0 Liters!', 'danger'); document.getElementById('milk-qty').focus(); return; }
  ...
  const waterPct = parseFloat(String(document.getElementById('milk-water-pct')?.value || '0').replace(/[^0-9.]/g, '')) || 0;
  const type     = document.getElementById('milk-type').value;
  ...
  const entry = {
    ...
    farmerId, farmerName: farmer ? farmer.name : 'Unknown', farmerEmail: farmer ? farmer.email : '',
    type, shift: document.getElementById('milk-shift').value,
    qty, liters: qty, fat, snf, waterPct, rate, total, totalAmount: total,
    agentEmail: currentAgentEmail || ''
  };
  DB.collections.push(entry);
  saveDB();
  sendMilkBillReceipt(entry);
  fetch(`${API_BASE_URL}/api/collections`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(entry) }).catch(err => console.warn('Collection cloud sync offline:', err));
...
document.getElementById('deduction-form')?.addEventListener('submit', function(e) {
  ...
  saveDB();
  fetch(`${API_BASE_URL}/api/deductions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(ded) }).catch(err => console.warn('Deduction cloud sync offline:', err));
...
document.getElementById('farmer-verification-form')?.addEventListener('submit', function(e) {
  ...
  if (existingEmail)  { showAlert(`Registration Error: Farmer with Gmail (${email}) is already registered! (Primary Key: ${existingEmail.id})`, 'danger'); return; }
  const existingMobile = DB.farmers.find(f => f.mobile === mobile);
  if (existingMobile) { showAlert(`Registration Error: Farmer with Mobile Number (${mobile}) is already registered! (Primary Key: ${existingMobile.id})`, 'danger'); return; }
  handleVerifyOTP('farmer-email', 'farmer-aadhaar-otp', () => {
    ...
    const newFarmer = {
      ...
      password:    'farmer123', // Default — server will send welcome email with this
      registeredBy: currentAgentEmail || 'Agent',
      agentEmail:  currentAgentEmail || ''
    };
    DB.farmers.push(newFarmer);
    saveDB();
    // POST to server — server auto-sends welcome email with credentials
    fetch(`${API_BASE_URL}/api/farmers`, {
      ...
      if (data.emailSent) {
        addAiLog('tag-security', 'AI-EMAIL', `Welcome email with credentials sent to ${newFarmer.email}`);
      }
    }).catch(err => console.warn('Farmer cloud sync offline:', err));
    showAlert(`✅ Farmer ${newFarmer.name} registered! A welcome email with portal login credentials has been sent to ${email}.`);
    addAiLog('tag-security', 'AI-REGISTER', `Farmer ${newFarmer.name} (PK: ${id}) registered. Welcome email dispatched.`);
    populateDropdowns();
    renderFarmers();
    this.reset();
...
function renderFarmers() {
  ...
  if (!tbody) return;
  const scopedFarmers = getScopedFarmers();
  if (scopedFarmers.length === 0) { tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;">No farmers registered yet.</td></tr>`; return; }
...
function exportFarmersToCsv() {
  ...
  scopedFarmers.forEach(f => { csv += `"${f.id}","${f.name}","${f.village}","${f.mobile}","${f.email}","${f.bankName}","${f.account}","${f.ifsc}"\n`; });
...
function renderCollections() {
  ...
  if (scopedCollections.length === 0) { tbody.innerHTML = `<tr><td colspan="10" style="text-align:center;">No milk collection entries found.</td></tr>`; return; }
...
function populateDropdowns() {
  ...
  ['milk-farmer-id', 'deduction-farmer-id', 'report-farmer-code'].forEach(id => {
    ...
    if (select) select.innerHTML = `<option value="">Select Farmer</option>` + options;
  });
}
...
function renderDashboardCharts() {
  ...
  const dailyTotals = dates.map(d => scopedCollections.filter(c => c.date === d).reduce((acc, curr) => acc + (parseFloat(curr.qty) || 0), 0));
  window.trendChartInst = new Chart(trendCtx, {
    type: 'line',
    data: { labels: dates.length ? dates : ['Today'], datasets: [{ label: 'Volume (Liters)', data: dailyTotals.length ? dailyTotals : [0], borderColor: '#1b4332', backgroundColor: 'rgba(27,67,50,0.1)', fill: true, tension: 0.3 }] },
    options: { responsive: true, maintainAspectRatio: false }
  });
  const cowCount     = scopedCollections.filter(c => c.type === 'Cow Milk').length;
  const buffaloCount = scopedCollections.filter(c => c.type === 'Buffalo Milk').length;
  window.qualityChartInst = new Chart(qualityCtx, {
    type: 'doughnut',
    data: { labels: ['Cow Milk', 'Buffalo Milk'], datasets: [{ data: [cowCount, buffaloCount], backgroundColor: ['#206a78', '#ffb703'] }] },
    options: { responsive: true, maintainAspectRatio: false }
  });
}
...
function renderFarmerCharts(collections = []) {
  ...
  const dateMap = {};
  let morningQty = 0, eveningQty = 0;
  collections.forEach(c => {
    const qty = parseFloat(c.qty) || 0;
    dateMap[c.date] = (dateMap[c.date] || 0) + qty;
    if ((c.shift || '').toLowerCase().includes('morn')) morningQty += qty; else eveningQty += qty;
  });
  ...
  farmerVolChartInst = new Chart(volCtx, {
    type: 'bar',
    data: { labels: sortedDates.length > 0 ? sortedDates : ['No Data'], datasets: [{ label: 'Daily Milk Volume (L)', data: volumes.length > 0 ? volumes : [0], backgroundColor: '#2d6a4f', borderColor: '#1b4332', borderWidth: 1, borderRadius: 4 }] },
    options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, title: { display: true, text: 'Liters (L)' } } } }
  });
  farmerShiftChartInst = new Chart(shiftCtx, {
    type: 'doughnut',
    data: { labels: ['Morning Shift', 'Evening Shift'], datasets: [{ data: [morningQty, eveningQty], backgroundColor: ['#ffb703', '#206a78'] }] },
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } }
  });
}
function renderFarmerPortal() {
  ...
  if (fromInput && !fromInput.value) { const d = new Date(); d.setDate(d.getDate() - 14); fromInput.value = getLocalDateString(d); }
  if (toInput   && !toInput.value)   toInput.value = getLocalDateString();
  ...
  if (!fromDate || !toDate) { datePromptCard?.classList.remove('hidden'); reportContainer?.classList.add('hidden'); renderFarmerBookings(); return; }
  ...
  const farmerCollections = (DB.collections || []).filter(c =>
    (c.farmerId === currentFarmer.id || c.farmerEmail === currentFarmer.email) && c.date >= fromDate && c.date <= toDate
  );
  let totalQty = 0, totalEarnings = 0;
  ...
  const farmerDeductions = (DB.bookings || []).filter(b =>
    (b.farmerId === currentFarmer.id || b.farmerEmail === currentFarmer.email) &&
    (b.status === 'Approved & Cost Deducted' || b.status === 'APPROVED') &&
...
function handleReportTypeChange(val) {
  ...
  if (val === 'SHIFT_SUMMARY') {
    groupDate?.classList.remove('hidden');    groupEndDate?.classList.remove('hidden');
    groupShift?.classList.remove('hidden');   groupFarmer?.classList.add('hidden');
  } else if (val === 'MILK_BILL') {
    groupDate?.classList.remove('hidden');    groupEndDate?.classList.remove('hidden');
    groupShift?.classList.add('hidden');      groupFarmer?.classList.remove('hidden');
  } else if (val === 'BILL_SUMMARY') {
    groupDate?.classList.remove('hidden');    groupEndDate?.classList.remove('hidden');
    groupShift?.classList.add('hidden');      groupFarmer?.classList.add('hidden');
  }
}
function generateSelectedReport() {
  ...
  if (!startDate || !endDate) { showAlert('Validation Error: Please select both From Date and To Date to generate the report.', 'danger'); return; }
  if (endDate < startDate)    { showAlert('Validation Error: To Date cannot be earlier than From Date.', 'danger'); return; }
  ...
  if (reportType === 'SHIFT_SUMMARY') {
    ...
    headElem.innerHTML  = `<tr><th>Farmer Code</th><th>Farmer Name</th><th>Shift</th><th>Milk Type</th><th>Qty (L)</th><th>FAT %</th><th>SNF %</th><th>Water %</th><th>Total (₹)</th></tr>`;
    const filtered = getScopedCollections().filter(c => c.date >= startDate && c.date <= endDate && (shift === 'ALL' || c.shift === shift));
    let totQty = 0, totAmt = 0;
    bodyElem.innerHTML = filtered.map(c => { const q = parseFloat(c.qty)||0; totQty+=q; totAmt+=parseFloat(c.total)||0; return `<tr><td>${c.farmerId}</td><td>${c.farmerName}</td><td>${c.shift}</td><td>${c.type}</td><td>${q} L</td><td>${c.fat}%</td><td>${c.snf}%</td><td>${c.waterPct}%</td><td>₹${c.total}</td></tr>`; }).join('') || `<tr><td colspan="9" style="text-align:center;">No records found between ${startDate} and ${endDate}.</td></tr>`;
    footElem.innerHTML = `<tr><td colspan="4">TOTALS</td><td>${totQty.toFixed(1)} L</td><td colspan="3"></td><td>₹${totAmt.toFixed(2)}</td></tr>`;
  } else if (reportType === 'MILK_BILL') {
    
    const farmer    = getScopedFarmers().find(f => f.id === farmerId);
    titleElem.innerText = `INDIVIDUAL MILK BILL - ${farmer ? farmer.name : farmerId}`;
    rangeElem.innerText = `${startDate} to ${endDate}`;
    headElem.innerHTML  = `<tr><th>Date</th><th>Shift</th><th>Type</th><th>Qty (L)</th><th>FAT %</th><th>SNF %</th><th>Rate/L</th><th>Amount (₹)</th></tr>`;
    const filtered = getScopedCollections().filter(c => c.farmerId === farmerId && c.date >= startDate && c.date <= endDate);
    let totQty = 0, totAmt = 0;
    bodyElem.innerHTML = filtered.map(c => { const q = parseFloat(c.qty)||0; totQty+=q; totAmt+=parseFloat(c.total)||0; return `<tr><td>${c.date}</td><td>${c.shift}</td><td>${c.type}</td><td>${q} L</td><td>${c.fat}%</td><td>${c.snf}%</td><td>₹${c.rate}</td><td>₹${c.total}</td></tr>`; }).join('') || `<tr><td colspan="8" style="text-align:center;">No records found between ${startDate} and ${endDate}.</td></tr>`;
    footElem.innerHTML = `<tr><td colspan="3">TOTAL PAYABLE</td><td>${totQty.toFixed(1)} L</td><td colspan="3"></td><td>₹${totAmt.toFixed(2)}</td></tr>`;
  } else if (reportType === 'BILL_SUMMARY') {
    titleElem.innerText = 'CONSOLIDATED BILL SUMMARY REPORT';
    rangeElem.innerText = `${startDate} to ${endDate}`;
    headElem.innerHTML  = `<tr><th>Farmer Code</th><th>Farmer Name</th><th>Total Volume (L)</th><th>Gross Earnings (₹)</th><th>Deductions (₹)</th><th>Net Payable (₹)</th></tr>`;
    ...
    bodyElem.innerHTML = scopedFarmers.map(f => {
      ...
      const fQty   = fColls.reduce((s, c) => s + (parseFloat(c.qty)   || 0), 0);
      const fGross = fColls.reduce((s, c) => s + (parseFloat(c.total) || 0), 0);
      const fDed   = fDeds.reduce( (s, b) => s + (parseFloat(b.totalPrice) || 0), 0);
      const fNet   = fGross - fDed;
      gQty += fQty; gGross += fGross; gDed += fDed; gNet += fNet;
      return `<tr><td>${f.id}</td><td>${f.name}</td><td>${fQty.toFixed(1)} L</td><td>₹${fGross.toFixed(2)}</td><td>₹${fDed.toFixed(2)}</td><td><strong>₹${fNet.toFixed(2)}</strong></td></tr>`;
    }).join('') || `<tr><td colspan="6" style="text-align:center;">No farmer records found between ${startDate} and ${endDate}.</td></tr>`;
    footElem.innerHTML = `<tr><td colspan="2">GRAND TOTALS</td><td>${gQty.toFixed(1)} L</td><td>₹${gGross.toFixed(2)}</td><td>₹${gDed.toFixed(2)}</td><td>₹${gNet.toFixed(2)}</td></tr>`;
  }
}
// ─────────────────────────────────────────────
// DOMContentLoaded — Wire up all forms
// ─────────────────────────────────────────────
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (bookingDateInput)  { bookingDateInput.value = today;      bookingDateInput.min = today; bookingDateInput.max = today; bookingDateInput.addEventListener('change', handleBookingDateChange); }
  if (deliveryDateInput) { deliveryDateInput.value = tomorrowStr; deliveryDateInput.min = tomorrowStr; }
  // ── AGENT LOGIN ──
  document.getElementById('agent-login-form')?.addEventListener('submit', async function(e) {
    ...
    if (agentAuthMode === 'password') {
      ...
      try {
        const res  = await fetch(`${API_BASE_URL}/api/agent/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password: pass }) });
        const data = await res.json();
        if (data.success) {
        ...
        if (storedPass && pass === storedPass) { currentAgentEmail = email; addAiLog('tag-security', 'AI-SECURITY', `Agent ${email} logged in offline.`); initAppView('agent'); }
        else showAlert('Server unreachable. If previously logged in, use your cached password.', 'danger');
      }
    } else {
  ...
  document.getElementById('agent-reset-form')?.addEventListener('submit', function(e) {
    ...
    handleVerifyOTP('reset-agent-email', 'reset-agent-otp', () => {
      ...
      fetch(`${API_BASE_URL}/api/agents`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password: newPass }) }).catch(err => console.warn('Agent registration sync offline:', err));
  ...
  // ── FARMER LOGIN (server-side verification via /api/farmer/verify-login) ──
  document.getElementById('farmer-login-form')?.addEventListener('submit', async function(e) {
    ...
    if (farmerAuthMode === 'password') {
      ...
      // ── Try server-side verification first (MongoDB password) ──
      try {
        const res  = await fetch(`${API_BASE_URL}/api/farmer/verify-login`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const data = await res.json();
        if (res.ok && data.success) {
          // Merge fresh data from server into local DB
          const serverFarmer = data.farmer;
          const idx = DB.farmers.findIndex(f => f.email === email);
          if (idx >= 0) DB.farmers[idx] = { ...DB.farmers[idx], ...serverFarmer };
          else DB.farmers.push(serverFarmer);
          saveDB();
          currentFarmer = DB.farmers.find(f => f.email === email) || serverFarmer;
          addAiLog('tag-security', 'AI-SECURITY', `Farmer ${currentFarmer.name} logged in via server-verified password.`);
          initAppView('farmer');
        } else {
          showAlert(data.message || 'Incorrect password. Use "Forgot Password" to reset.', 'danger');
          addAiLog('tag-security', 'AI-SECURITY', `Failed farmer login for ${email}: ${data.message}`);
        }
      } catch (err) {
        // ── Offline fallback: check local DB ──
        const localFarmer = DB.farmers.find(f => f.email === email);
        if (!localFarmer) { showAlert('No registered farmer found with this email (offline mode — check connection).', 'danger'); return; }
        if (pass === (localFarmer.password || 'farmer123')) {
          currentFarmer = localFarmer;
          addAiLog('tag-security', 'AI-SECURITY', `Farmer ${localFarmer.name} logged in offline (cached password).`);
          initAppView('farmer');
        } else {
          showAlert('Incorrect password. Use "Forgot Password" to reset it.', 'danger');
        }
      }
    } else {
      // OTP mode — still check if farmer exists first
      const farmer = DB.farmers.find(f => f.email === email);
      if (!farmer) {
        try {
          const res  = await fetch(`${API_BASE_URL}/api/farmer/verify-login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password: '' }) });
          const data = await res.json();
          if (!res.ok && res.status === 404) { showAlert(data.message || 'No farmer account found with this email.', 'danger'); return; }
        } catch (_) {
          showAlert('No registered farmer found with this Gmail address.', 'danger');
          return;
        }
      }
      handleVerifyOTP('farmer-login-email', 'farmer-login-otp', async () => {
        // Fetch fresh farmer data from server on OTP login
        try {
          const res  = await fetch(`${API_BASE_URL}/api/farmer/verify-login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password: farmer?.password || 'farmer123' }) });
          const data = await res.json();
          if (res.ok && data.success) {
            const idx = DB.farmers.findIndex(f => f.email === email);
            if (idx >= 0) DB.farmers[idx] = { ...DB.farmers[idx], ...data.farmer }; else DB.farmers.push(data.farmer);
            saveDB();
            currentFarmer = DB.farmers.find(f => f.email === email) || data.farmer;
          } else { currentFarmer = farmer; }
        } catch (_) { currentFarmer = farmer; }
        addAiLog('tag-security', 'AI-SECURITY', `Farmer ${currentFarmer?.name || email} logged in via OTP.`);
        initAppView('farmer');
      });
    }
  });
  // ── FARMER RESET PASSWORD FORM
  // sendFarmerResetOtp() called from HTML button
  // saveFarmerResetPassword() called from HTML button
  // (Both are defined as standalone async functions above)
  // ── RATE SETTINGS ──
  document.getElementById('rate-settings-form')?.addEventListener('submit', function(e) {
    ...
    fetch(`${API_BASE_URL}/api/rates`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ agentEmail: currentAgentEmail || '', ...rates }) }).catch(err => console.warn('Rate config sync offline:', err));
    updateNoticeBoardRates();
  ...
  document.getElementById('farmer-feedback-form')?.addEventListener('submit', function(e) {
    ...
    if (!text) { showAlert('Please enter your feedback text.', 'danger'); return; }
    const fb = { id: `FB-${Date.now()}`, farmerId: currentFarmer.id, farmerName: currentFarmer.name, farmerEmail: currentFarmer.email, rating, text, createdAt: new Date().toISOString() };
    if (!DB.feedbacks) DB.feedbacks = [];
    DB.feedbacks.push(fb);
    saveDB();
    fetch(`${API_BASE_URL}/api/feedbacks`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(fb) }).catch(err => console.warn('Feedback sync offline:', err));
  ...
  document.getElementById('station-config-form')?.addEventListener('submit', function(e) {
    ...
    if (!newCAN) { showAlert('Please enter a valid CAN number.', 'danger'); return; }
    const cfg = getAgentConfig(currentAgentEmail);
    cfg.can = newCAN; cfg.village = newVillage; cfg.cycle = newCycle;
    DB.agentCAN = newCAN;
    ...
    saveDB();
    updateCanDisplays();
    fetch(`${API_BASE_URL}/api/station-config`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ agentEmail: currentAgentEmail || '', can: newCAN, village: newVillage, cycle: newCycle }) }).catch(err => console.warn('Station config sync offline:', err));
  ...
  document.getElementById('agent-can-input')?.addEventListener('change', function() {
    ...
    if (newCAN && currentAgentEmail) {
      const cfg = getAgentConfig(currentAgentEmail);
      cfg.can = newCAN; DB.agentCAN = newCAN; saveDB(); updateCanDisplays();
      fetch(`${API_BASE_URL}/api/station-config`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ agentEmail: currentAgentEmail, can: newCAN, village: cfg.village, cycle: cfg.cycle }) }).catch(err => console.warn('Header CAN sync offline:', err));
  ...
  document.getElementById('admin-login-form')?.addEventListener('submit', async function(e) {
    ...
    if (!captchaInput || captchaInput !== captchaBox) { showAlert('❌ CAPTCHA verification failed. Please try again.', 'danger'); generateCaptcha('admin'); return; }
    try {
      const res  = await fetch(`${API_BASE_URL}/api/admin/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
      const data = await res.json();
      if (!data.success) { showAlert(`❌ ${data.message}`, 'danger'); generateCaptcha('admin'); return; }
    ...
    } catch (err) { showAlert('Admin login failed — server unreachable.', 'danger'); }
  });
  // Auto sync every 5 seconds while logged in
  setInterval(() => { if (currentAgentEmail || currentFarmer) syncFromMongoDB(true); }, 5000);
});
